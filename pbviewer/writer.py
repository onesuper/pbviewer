...
try:
    from pywirebuf_encoder import Encoder  # cpp module
except ImportError:
    from encoder import Encoder  # python module
...
class ProtobufWriter:
    ...
    def write_tag(self, field_num, wire_type):
        self._encoder.AppendTag(field_num, wire_type)
        self._refresh_buffer()

    def write_long(self, val):
        self._encoder.AppendSInt64(val)
        self._refresh_buffer()

    def write_uint(self, val):
        self._encoder.AppendUInt32(val)
        self._refresh_buffer()

    def write_bool(self, val):
        self._encoder.AppendBool(val)
        self._refresh_buffer()

    def write_double(self, val):
        self._encoder.AppendDouble(val)
        self._refresh_buffer()

    def write_string(self, val):
        self._encoder.AppendString(val)
        self._refresh_buffer()
